...
class Se3Clusterer:
    """
        A class to perform self-segmentation of legal documents using a Transformer model and semantic similarity.

    The Se3Clusterer applies a segmentation method where documents are split into chunks based on their semantic
    similarity. The class uses a pre-trained Transformer model to encode sentences and then assigns them to chunks that
    maximize semantic coherence.

    Attributes:
        tokenizer (AutoTokenizer): Tokenizer from the pre-trained Transformer model.
        model (AutoModel): Pre-trained Transformer model used to encode sentences.
    ...
    def __init__(self):
        """
        Initializes the Se3Clusterer with a tokenizer and a pre-trained Transformer model for encoding sentences.
        """
